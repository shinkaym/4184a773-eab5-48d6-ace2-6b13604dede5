  ...
  return <div className="min-h-screen w-full bg-nature-main flex flex-col font-sans text-nature-text-primary">
      ...
      <div className="flex-grow w-full max-w-[1400px] mx-auto p-6 md:py-12 grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-9">
          <motion.div variants={containerVariants} initial="hidden" animate="visible" className="relative border-4 border-nature-text-primary p-8 md:p-12 bg-white shadow-xl">
            {/* Step Label Tag */}
            ...
            <motion.h1 variants={itemVariants} className="text-3xl md:text-4xl font-black tracking-tighter uppercase mb-12">
              Select Appointment
            </motion.h1>

            {/* Client Info Section */}
            <motion.div variants={itemVariants} className="mb-10 p-4 bg-nature-surface/30 border-l-4 border-nature-primary">
              <div className="flex items-center gap-4">
                ...
                <button className="text-xs font-bold uppercase tracking-wider underline hover:text-nature-primary transition-colors">
                  Edit
                </button>
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="mb-10">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="relative">
                  ...
                  <input type="text" placeholder="Search staff..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 pr-4 py-2 border-2 border-nature-divider rounded-lg text-sm focus:border-nature-primary focus:outline-none transition-colors" />
                </div>
              </div>

              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {staffMembers.map(staff => <StaffCard key={staff.id} name={staff.name} role={staff.role} image={staff.image} isAnyStaff={staff.id === 'any'} isSelected={selectedStaff === staff.id} onClick={() => setSelectedStaff(staff.id)} />)}
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="grid md:grid-cols-2 gap-8 mb-10">
              ...
              <div>
                ...
                <div className="border-2 border-nature-divider rounded-lg p-6 bg-white">
                  ...
                  <div className="grid grid-cols-7 gap-2">
                    {['SU', 'MO', 'TU', 'WE', 'TH', 'FR', 'SA'].map(day => <div key={day} className="text-center text-[10px] font-mono text-nature-text-tertiary uppercase font-bold">
                        {day}
                      </div>)}
                    ...
                    return <button key={i} disabled={isDisabled} onClick={() => setSelectedDate(date)} className={`
              ...
              <div>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="text-xs font-mono text-nature-text-tertiary uppercase tracking-wider">
                    Jan 15
                  </span>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      ...
                      return <button key={time} disabled={isDisabled} onClick={() => setSelectedTime(time)} className={`
                              py-3 px-2 text-xs font-mono border-2 rounded transition-all
                              ${isSelected ? 'bg-nature-text-primary text-white border-nature-text-primary font-bold' : ''}
                              ${isDisabled ? 'bg-nature-surface/50 text-nature-text-tertiary/30 border-nature-divider/30 cursor-not-allowed line-through' : ''}
                              ${!isSelected && !isDisabled ? 'border-nature-divider hover:border-nature-text-primary hover:bg-nature-surface' : ''}
                            `}>
                            {time}
                          </button>;
                    })}
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      ...
                      const isSelected = time === selectedTime;
                      return <button key={time} onClick={() => setSelectedTime(time)} className={`
                              py-3 px-2 text-xs font-mono border-2 rounded transition-all
                              ${isSelected ? 'bg-nature-text-primary text-white border-nature-text-primary font-bold' : 'border-nature-divider hover:border-nature-text-primary hover:bg-nature-surface'}
                            `}>
                            {time}
                          </button>;
                    })}
            ...
            <motion.div variants={itemVariants} className="flex justify-end">
              ...
              scale: 0.98
            }} onClick={onNext} className="bg-nature-text-primary text-white px-8 py-4 flex items-center gap-3 font-bold uppercase tracking-widest hover:bg-nature-text-secondary transition-colors shadow-lg">
                Next Step
